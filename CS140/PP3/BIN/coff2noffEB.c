...
{
  int fdIn, fdOut, numsections, i, textCoffStart, textNoffStart;
  ...
  if (fileh.f_magic != MIPSEBMAGIC) {
    fprintf(stderr, "File is not a MIPSEB COFF file\n");
    unlink(noffFileName);
    exit(1);
  }
  ...
  for (i = 0; i < numsections; i++) {
    ...
    if (!strcmp(sections[i].s_name, ".text")) {
      textCoffStart = sections[i].s_scnptr;
    }
  }
   ...
   * in the COFF file
   */
  bzero(&noffH, sizeof(noffH));         /* zero out the entire noff Header */
  noffH.noffMagic = NOFFMAGIC;
  textNoffStart = sizeof(NoffHeader);
  ...
  for (i = 0; i < numsections; i++) {
    ...
    if (sections[i].s_size == 0) {
      ...
      noffH.code.inFileAddr = sections[i].s_scnptr - textCoffStart + textNoffStart;
      ...
      lseek(fdOut, noffH.code.inFileAddr, 0);
      Write(fdOut, buffer, sections[i].s_size);
      free(buffer);
    } else if (!strcmp(sections[i].s_name, ".rdata")) {
      if (sections[i].s_paddr != (noffH.code.virtualAddr + noffH.code.size)) {
	fprintf(stderr, "rdata (%x) must start right after text end in VA (%x).\n",
		sections[i].s_paddr, noffH.code.virtualAddr + noffH.code.size);
	unlink(noffFileName);
	exit(1);
      }
      lseek(fdIn, sections[i].s_scnptr, 0);
      buffer = malloc(sections[i].s_size);
      Read(fdIn, buffer, sections[i].s_size);
      lseek(fdOut, noffH.code.inFileAddr + noffH.code.size, 0);
      Write(fdOut, buffer, sections[i].s_size);
      free(buffer);
      noffH.code.size += sections[i].s_size;
    } else if (!strcmp(sections[i].s_name, ".data")) {
      noffH.initData.virtualAddr = sections[i].s_paddr;
      noffH.initData.inFileAddr = sections[i].s_scnptr - textCoffStart + textNoffStart;
      noffH.initData.size = sections[i].s_size;
      lseek(fdIn, sections[i].s_scnptr, 0);
      buffer = malloc(sections[i].s_size);
      Read(fdIn, buffer, sections[i].s_size);
      lseek(fdOut, noffH.initData.inFileAddr, 0);
      Write(fdOut, buffer, sections[i].s_size);
      free(buffer);
